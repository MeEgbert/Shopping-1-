...
<body>
<!--导航栏部分-->
<jsp:include page="include/handle_Head.jsp"/>
        ...
        for(var i=0;i<allShoppingRecords.length;i++){
            ...
            if(allShoppingRecords[i].orderStatus == 0){
                unHandleHTMLTemp+= '<tr>'+
                        ...
                        '<td>'+orderArray[allShoppingRecords[i].orderStatus]+'</td>'+
                        '<td>'+
                        '<button class="btn btn-success btn-sm" onclick="sendProducts('+allShoppingRecords[i].userId+','+allShoppingRecords[i].productId+',\''+allShoppingRecords[i].time+'\')">发货</button>'+
                        '</td>'+
                        '</tr>';
                unHandleCounts++;
            }
